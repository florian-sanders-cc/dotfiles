...
(async function () {
  ...
  const currentTokens = await fs.readFile(CONFIG_PATH + '/clever-tools.json', 'utf8')
    .then((json) => {
      return JSON.parse(json);
    })
    .catch(() => null);

  const result = spawnSync('clever', ['curl', 'https://api.clever-cloud.com/v2/self']);
  ...
  const profiles = await fs.readFile(CONFIG_PATH + '/profiles.json', 'utf8')
    .then((json) => {
      return JSON.parse(json);
    })
    .catch(() => []);

  if (currentProfile != null && profiles.find(({ id }) => id === currentProfile.id) == null) {
  ...
  await fs.writeFile(CONFIG_PATH + '/profiles.json', JSON.stringify(profiles, null, 2));
  ...
  if (choices.length === 0) {
    ...
    if (profileToSwitch == null) {
      ...
      await fs.writeFile(CONFIG_PATH + '/clever-tools.json', JSON.stringify(tokens, null, 2));
